...
/*   Updated: 2023/12/13 15:34:44 by sbaumann         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*extract_line(char *txt)
{
	...
	while (txt[count] && txt[count] != '\n')
		count++;
	res = (char *)malloc(count * sizeof(char) + 2);
	if (!res)
		return (NULL);
	count = 0;
	while (txt[count] && txt[count] != '\n')
...
char	*get_next_line(int fd)
{
	...
	current_line[fd] = read_line(fd, current_line[fd]);
	if (!current_line[fd])
		return (NULL);
	dest = extract_line(current_line[fd]);
	current_line[fd] = remove_line(current_line[fd]);
	return (dest);
}
